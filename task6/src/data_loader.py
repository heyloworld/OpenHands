...
class DataLoader:
    ...
    def load_data(self):
        """
        Load the Sentiment140 dataset from Hugging Face.
        ...
        try:
            ...
            # Map target values (0 = negative, 4 = positive) to binary (0 = negative, 1 = positive)
            train_df['target'] = train_df['target'].map({0: 0, 4: 1})
            
            # Take a sample if specified
            if self.sample_size is not None:
                train_df = train_df.sample(n=self.sample_size, random_state=self.random_state)
            
            logger.info(f"Dataset loaded successfully with {len(train_df)} samples.")
            return train_df
